...
const placement = computed<Placement>(() => {
  ...
  const { offsetWidth: docWidth, offsetHeight: docHeight } = doc.value || { offsetWidth: 0, offsetHeight: 0 };
...
<template>
  <Teleport to="body">
    <div class="guider" :style="`display: ${show ? 'static' : 'none'}`">
      ...
      <div class="guider-top"
        :style="`left: ${location.left - 2}px; width: ${location.width + 4}px; border-top: ${location.top - 2}px solid rgba(0, 0, 0, 0.25);`">
      </div>
      <div class="guider-right"
        :style="`left: ${location.left + location.width + 2}px; background-color: rgba(0, 0, 0, 0.25)`"></div>
      <div class="guider-bottom"
        :style="`left: ${location.left - 2}px; width: ${location.width + 4}px; top: ${location.top + location.height + 2}px`">
      </div>
      <div ref="doc" class="guider-doc flex flex-col justify-between"
        :style="`left: ${docPosition.left}px;top:${docPosition.top}px`">
        <div class="arrow"
          :style="`left: ${arrowStyle.left}px; top: ${arrowStyle.top}px; border-${placement[placement.main]}-color: white`">
        </div>
